...
function buyDynamicUp(num) {
  if (num == 0) {
    var costThis = EN.mul(1e10, EN.pow(10, EN.pow(game.dynamicLevel, 1.5)));
    if (EN.gte(game.OP, costThis)) {
      game.OP = EN.sub(game.OP, costThis);
      game.dynamicLevel = EN.add(game.dynamicLevel, 1);
    }
  } else if (num == 1) {
    var costThis = EN.mul(1e10, EN.pow(10, EN.pow(game.dynamicLevel2, 1.5)));
    if (EN.gte(game.OP, costThis)) {
      game.OP = EN.sub(game.OP, costThis);
...
function changeOrdLengthLess() {
  let newms = prompt(
    "Please type in the new max length. Type in 0 for no maximum (maximum 48)"
  );
  if (!isNaN(Number(newms))) {
	  if(newms >= 48 || newms <= 0) {
    game.maxOrdLength.less = 48;
  }
...
function displayOrd(ord,base=3,over=0,trim=0,large=0,multoff=0,colour=0) {
  ...
  while (ord.gte(base) && (trim < game.maxOrdLength.less || game.maxOrdLength.less == 0) && !largeOrd)
  {
    ...
    dispString += (colour==1?"<span style='color:" + (tempvar*7.5 > 1.79769e308 ? HSL(0) : HSL(tempvar*7.5)) + ";text-shadow: 6px 6px 6px " + (tempvar*7.5 > 1.79769e308 ? HSL(0) : HSL(tempvar*7.5)) + ", 1px 0 1px black, -1px 0 1px black, 0 1px 1px black, 0 -1px 1px black;'>" + tempvar4 + "</span>":tempvar4);
    ord = ott;
    trim++;
  ...
  if ((ord.lt(base) && !ord.eq(0) && trim != game.maxOrdLength.less) || originalOrd.eq(0)) {
	...
	dispString = dispString.replace("+0", "")
  return dispString;
}
